...
// Captura secuencial (frontend/pages/captura-secuencial.js, compartida con
// la subida de facturas multipágina): un ticket térmico largo no cabe en
// una foto, así que se hacen varias de arriba abajo (con solape deliberado
// entre una y la siguiente) y se procesan todas juntas — ver el prompt
// multi-imagen en backend/lib/ventas.js.
let ventaPreviewState = null;
let ventaArchivosOriginales = [];
let capturaCierreCaja = null;

function initVentasScan() {
  if (!document.getElementById('btn-cierre-caja')) return; // esta página no tiene el flujo de ventas
  capturaCierreCaja = crearCapturaSecuencial({
    ids: {
      home: 'scan-home', captura: 'venta-captura', thumbs: 'venta-captura-thumbs', peso: 'venta-captura-peso',
      btnFoto: 'btn-hacer-foto', btnGaleria: 'btn-cierre-galeria', btnPdf: 'btn-cierre-pdf',
      inputCamara: 'input-cierre-camara', inputGaleria: 'input-cierre-galeria', inputPdf: 'input-cierre-pdf',
      btnProcesar: 'btn-procesar-cierre', btnCancelar: 'btn-cancelar-captura',
    },
    onProcesar: procesarCierreCaja,
  });
  document.getElementById('btn-cierre-caja').addEventListener('click', () => capturaCierreCaja.abrir());
}
document.addEventListener('DOMContentLoaded', initVentasScan);

// El envío conserva el orden de captura — crearCapturaSecuencial no reordena.
async function procesarCierreCaja(archivos) {
