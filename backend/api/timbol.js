// Timbol invoice module: manual upload via Claude Vision + Drive archive.
...
// Token = base64('usuario:timestamp'). requireAuth lo decodifica y consulta TIMBOL_USERS.
...
router.use(createInventarioRouter({ cliente: 'timbol', requireAuth, requireRole }));

// Escandallo asistido — módulo reutilizable (ver backend/api/escandallo.js).
// Sin prefijo: las rutas ya incluyen /escandallo.
router.use(createEscandalloRouter({ cliente: 'timbol', requireAuth, requireRole }));
...
router.use(createGastosPersonalRouter({ cliente: 'timbol', requireAuth, requireRole }));

// ── Claude Vision extraction ────────────────────────────────────────────────
// La extracción y su esquema viven en backend/lib/extraccion.js, compartidos
// con Comarea — ver extraerFactura().
...
async function trackTokens(operacion, usage, usuario) {
  ...
  const { error } = await supabase.from('timbol_token_usage').insert({ operacion, input_tokens: input, output_tokens: output, coste_euros: coste, usuario });
  if (error) console.error('[timbol] token tracking:', error.message);
}
...
router.post('/facturas/upload', requireAuth, upload.array('facturas', 10), async (req, res) => {
  ...
  try {
    ...
    const { data, usage } = await extraerFactura(archivos, { cliente: 'timbol' });
    ...
    // O2MAD Facturas / Clientes Externos / Timbol / YYYY / MM-NombreMes
    const rootId = process.env.DRIVE_ROOT_FOLDER_ID || null;
    const pathNames = rootId
      ? ['Clientes Externos', 'Timbol', year, monthFolder]
      : ['O2MAD Facturas', 'Clientes Externos', 'Timbol', year, monthFolder];
    ...
    const { data: saved, error: dbError } = await supabase
      .from('timbol_facturas').insert(row).select().single();
    ...
    if (rows.length) {
      ...
      if (lineasError) console.error('[timbol] guardar líneas:', lineasError.message);
    }

    console.log(`[timbol] ✓ ${data.numero_factura} — ${data.proveedor} (${data.importe_total ?? 's/imp'}) · ${rows.length} línea(s) · verif=${lineasVerificadas}`);
    res.json({ ok: true, factura: { ...saved, lineas: rows } });
  } catch (e) {
    console.error('[timbol] upload error:', e.message);
...
// DELETE /timbol/facturas/:id — solo gestor/admin
router.delete('/facturas/:id', requireAuth, requireRole('gestor', 'admin'), async (req, res) => {
  ...
  try {
    const { data: factura, error: findError } = await supabase
      .from('timbol_facturas').select('id, drive_file_id, proveedor').eq('id', id).single();
    ...
    if (factura.drive_file_id) {
      try {
        ...
        console.warn(`[timbol] no se pudo borrar el archivo de Drive (${factura.drive_file_id}): ${e.message}`);
      }
    }

    const { error: delError } = await supabase.from('timbol_facturas').delete().eq('id', id);
    if (delError) throw new Error(`Supabase: ${delError.message}`);

    console.log(`[timbol] ✗ factura ${id} borrada — ${factura.proveedor} (drive_deleted=${driveDeleted})`);
    res.json({ success: true, id, drive_deleted: driveDeleted });
  } catch (e) {
    console.error('[timbol] delete error:', e.message);
...
// PATCH /timbol/facturas/:id/tipo — corrección manual del tipo, solo gestor/admin
router.patch('/facturas/:id/tipo', requireAuth, requireRole('gestor', 'admin'), async (req, res) => {
  ...
  try {
    ...
    console.log(`[timbol] tipo corregido manualmente: factura ${id} → ${tipo} (${req.user.email})`);
...
// PATCH /timbol/facturas/:id — edición en línea de fecha/número desde el
...
router.patch('/facturas/:id', requireAuth, requireRole('gestor', 'admin'), async (req, res) => {
  ...
  try {
    const { data: saved, error } = await supabase
      .from('timbol_facturas').update(patch).eq('id', id).select().single();
    if (error) throw new Error(`Supabase: ${error.message}`);
    if (!saved) return res.status(404).json({ error: 'Factura no encontrada' });
    console.log(`[timbol] campo corregido manualmente: factura ${id} (${req.user.email})`);
    res.json({ ok: true, factura: saved });
  } catch (e) {
    console.error('[timbol] corregir campo error:', e.message);
    res.status(500).json({ error: e.message });
  }
});
...
router.get('/analytics', requireAuth, async (req, res) => {
  ...
  let q = supabase.from('timbol_facturas').select('mes, anyo, importe_total, importe_base, proveedor, tipo, fecha_factura').limit(10000);
  if (anyo) q = q.eq('anyo', Number(anyo));
  const { data, error } = await q;
  if (error) return res.status(500).json({ error: error.message });
...
// GET /timbol/analytics/foodcost?desde=&hasta= — compras (factura+ticket,
...
router.get('/analytics/foodcost', requireAuth, async (req, res) => {
  ...
  try {
    const { data: facturas, error: errF } = await supabase
      .from('timbol_facturas').select('fecha_factura, importe_total')
    ...
    const { data: ventas, error: errV } = await supabase
      ...
      .eq('cliente', 'timbol').gte('fecha', desde).lte('fecha', hasta);
    ...
    // Rango de fechas completo (no solo las que tienen datos) para que la
    // media móvil no salte huecos si un día no tiene compras ni ventas.
    const fechas = [];
...
router.get('/tokens', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  const { data: facturas } = await supabase
    .from('timbol_facturas').select('id', { count: 'exact', head: true });
